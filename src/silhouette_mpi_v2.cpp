...
int main(int argc, char* argv[])
{
	...
	//Send X Matrix & intervals Matrix to all the slave nodes [1,2,3...]
	if(rank == 0)
	{
		...
		while(slave_rank < np)
		{
			MPI_Send(X.data(),X.size(), MPI_DOUBLE, slave_rank, 4 , MPI_COMM_WORLD);
			MPI_Send(intervals.data(),intervals.size(), MPI_DOUBLE, slave_rank, 4 , MPI_COMM_WORLD);
			slave_rank++;
		}
	}

	//Receive X Matrix and intervals Matrix at all the slave nodes
	if (rank > 0)
	{
		X.resize(rows,cols);
		intervals.resize(nol*2,1);
		MPI_Recv(X.data(),(rows*cols), MPI_DOUBLE, 0, 4, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(intervals.data(),(nol*2), MPI_DOUBLE, 0, 4, MPI_COMM_WORLD, MPI_STATUS_IGNORE);	
	}
	...
	//Calculate a at the slave nodes and send the result to master
	if(rank > 0)
	{
	...
	//Receive the 'a' Matrix at the master
	if(rank == 0)
	{
	...
	//Compute the b matrix at the slave nodes & send the result to master
	if (rank > 0)
	{
	int offset = (rank-1) * (intervals.size()/(np-1));
	int pkt_size = intervals.size()/(np-1);
	...
	//receive the b matrix at the master node
	if(rank == 0)
	{
		//receive the packet size - offset the pointer - receive the data//
